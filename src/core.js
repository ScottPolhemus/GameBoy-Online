...
import { fromTypedArray, getTypedArray, toTypedArray } from './utils'
import XAudioServer from './XAudioServer'

export default class GameBoyPlayerCore {
  constructor(params = {}) {
    const {
      canvas,
      loadSRAM,
    ...
    this.GBBOOTROM = GBBOOTROM || [] // GB BOOT ROM
    this.GBCBOOTROM = GBCBOOTROM || [] // GBC BOOT ROM
  ...
  openSRAMState(state) {
    ...
    this.MBCRam = toTypedArray(state, "uint8");
  }
  ...
  initBootstrap() {
    ...
    // Set the joypad state.
    this.memory[0xFF00] = 0xF;
  }

  // Load the ROM into memory and get cartridge information from it.
  async openROM(ROMImage) {
    // Load the first two ROM banks (0x0000 - 0x7FFF) into regular gameboy memory:
    this.ROM = [];
    this.usedBootROM = settings.bootBootRomFirst && ((!settings.forceGBBootRom && this.GBCBOOTROM.length == 0x800) || (settings.forceGBBootRom && this.GBBOOTROM.length == 0x100));
    ...
    if (this.usedBootROM) {
      if (!settings.forceGBBootRom) {
        ...
        for (; romIndex < 0x100; ++romIndex) {
          ...
          this.ROM[romIndex] = (ROMImage.charCodeAt(romIndex) & 0xFF); // Decode the ROM binary for the switch out.
        }
        for (; romIndex < 0x200; ++romIndex) {
          this.memory[romIndex] = this.ROM[romIndex] = (ROMImage.charCodeAt(romIndex) & 0xFF); // Load in the game ROM.
        }
    ...
    for (; romIndex < maxLength; ++romIndex) {
      this.ROM[romIndex] = (ROMImage.charCodeAt(romIndex) & 0xFF);
    }
    this.ROMBankEdge = Math.floor(this.ROM.length / 0x4000);
    // Set up the emulator for the cartidge specifics:
    await this.interpretCartridge(ROMImage);
  ...
  get ROMImage() {
    ...
    for (var index = 0; index < length; index++) {
      ROMImage += String.fromCharCode(this.ROM[index]);
  ...
  async interpretCartridge(ROMImage) {
    // ROM name
    this.name = ''
    this.gameCode = ''
    for (var index = 0x134; index < 0x13F; index++) {
      if (ROMImage.charCodeAt(index) > 0) {
        this.name += ROMImage[index];
      }
    }
    // ROM game code (for newer games)
    for (var index = 0x13F; index < 0x143; index++) {
      if (ROMImage.charCodeAt(index) > 0) {
        this.gameCode += ROMImage[index];
      }
    }

    cout("Game Title: " + this.name + "[" + this.gameCode + "][" + ROMImage[0x143] + "]", 0);
    ...
    if (!this.usedBootROM) {
      switch (this.ROM[0x143]) {
        ...
        case 0x32: // Exception to the GBC identifying code:
          if (!settings.gbHasPriority && this.name + this.gameCode + this.ROM[0x143] == "Game and Watch 50") {
            this.cGBC = true;
      ...
      await this.setupRAM(); // CPU/(V)RAM initialization.
  ...
  async setupRAM() {
    ...
    if (this.numRAMBanks > 0) {
      ...
      // Switched RAM Used
      var MBCRam = (typeof this.loadSRAM == "function") ? await this.loadSRAM(this.name) : [];
      if (MBCRam && MBCRam.length > 0) {
        // Flash the SRAM into memory:
        this.openSRAMState(MBCRam)
      } else {
    ...
    if (typeof this.loadRTC == "function" && this.cTIMER) {
      var rtcData = await this.loadRTC(this.name);
      if (rtcData) {
        this.openRTCState(rtcData);
      }
    }
  ...
  initLCD() {
    ...
    this.screenRGBCount = this.screenWidth * this.screenHeight * 4;
    ...
    try {
      ...
      try {
        this.canvasBuffer = this.drawContext.createImageData(this.screenWidth, this.screenHeight);
      } catch (error) {
        console.log("Falling back to the getImageData initialization (Error \"" + error.message + "\").");
        this.canvasBuffer = this.drawContext.getImageData(0, 0, this.screenWidth, this.screenHeight);
      }

      var index = this.screenRGBCount;
      while (index > 0) {
  ...
  initAudioBuffer() {
    ...
    this.downsampleInput = 0;
    this.bufferContainAmount = Math.max(this.baseCPUCyclesPerIteration * settings.audioBufferMinSpan / this.audioResamplerFirstPassFactor, 4096) << 1;
  ...
  processDraw(frameBuffer) {
    var canvasRGBALength = this.screenRGBCount;
  ...
  */
  ...
  memoryReadMBC(parentObj, address) {
    //Switchable RAM
    if (parentObj.MBCRAMBanksEnabled || settings.alwaysAllowRWtoBanks) {
  ...
  memoryWriteJumpCompile() {
    ...
    for (var index = 0x0000; index <= 0xFFFF; index++) {
      if (index < 0x8000) {
        ...
        if ((this.numRAMBanks == 1 / 16 && index < 0xA200) || this.numRAMBanks >= 1) {
          if (!this.cMBC3) {
            this.memoryWriter[index] = this.memoryWriteMBCRAM.bind(this);
          } else {
            //MBC3 RTC + RAM:
            this.memoryWriter[index] = this.memoryWriteMBC3RAM.bind(this);
          }
        } else {
          this.memoryWriter[index] = this.cartIgnoreWrite;
        }
